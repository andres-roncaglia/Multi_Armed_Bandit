...
Siendo X variable aleatoria que representa la cantidad de unidades monetarias ganadas en total en un año de juego y estimando las $P(X_\epsilon>X_{\epsilon'})$ para los $\epsilon$ planteados, concluímos que el valor de  $\epsilon/P(X_\epsilon>X_{\epsilon'})>P(X_\epsilon<X_{\epsilon'}) \ \ \ \ \forall{\epsilon'\neq\epsilon}$ es $\epsilon = 0,125$. Con este valor de $\epsilon$ es más probable obtener una mayor cantidad de unidades monetarias totales que con otros valores.
...
Desventajas de la estrategia:
  ...
  - Suele tardar en encontrar la mejor máquina.
...
#| fig-subcap: "El triángulo representa la media de las ganancias."
...
data.frame(Estrategia = factor(rep(colnames(estrategias_final), each = 1000), levels = colnames(estrategias_final)),
  ...
  labs(y = "Unidades monetarias ganadas en un año") +
