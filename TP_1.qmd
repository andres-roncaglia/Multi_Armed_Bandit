...
```{r e_greedy maximizacion de epsilon}
secuencia <- seq(0, 1, length.out = 50)
ganancias_esperadas <- NULL
posicion <- 0

for(i in secuencia) {
  posicion <- posicion + 1
  temp <- simulacion("e_greedy", n = 1000, param = i)
  ganancias_esperadas[posicion] <- round(sum(as.matrix(temp$Ganancias))/1000, 2)
  cat(i*100, "\n")
}

names(ganancias_esperadas) <- secuencia

ggplot() +
  aes(x = secuencia, y = ganancias_esperadas) +
  geom_line() +
  geom_vline(xintercept = secuencia[which.max(ganancias_esperadas)])
